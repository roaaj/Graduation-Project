...
<body>
    ...
    <section class="Best">
        ...
        <div class="clear"></div>
        <?php
        $res = mysqli_query($conn, "SELECT * FROM books WHERE store='" . $_SESSION['x'] . "' ORDER BY id ASC LIMIT $start_from, $limit");
        if (isset($res)) {
    ...
    <?php
    $result_db = mysqli_query($conn, "SELECT COUNT(id) FROM books WHERE store='" . $_SESSION['x'] . "'");
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        $pagLink .= "<li class='page-item'><a class='page-link' href='http://localhost/final%20p/store.php?store=" . $_SESSION['x'] . "&page=" . $i . "'>" . $i . "</a></li>";
    }
    echo $pagLink . "</ul>";
    ?>
