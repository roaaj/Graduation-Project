...
$res = mysqli_query($conn, "SELECT * FROM books WHERE distribution='B' ORDER BY id ASC LIMIT $start_from, $limit");
// 
...
<body>
    <section class="Best">
        <div class=" title">
            <h1 class="product_text"><strong><span class="den">Poetic</span>literature</strong></h1>
        </div>
        <div class="clear"></div>
        <?php
        if (isset($res)) {
    ...
    <?php
    $result_db = mysqli_query($conn, "SELECT COUNT(id) FROM books  WHERE distribution='B'");
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        $pagLink .= "<li class='page-item'><a class='page-link' href='Poeticliterature.php?page=" . $i . "'>" . $i . "</a></li>";
    }
    echo $pagLink . "</ul>";
    ?>
