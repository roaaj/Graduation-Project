...
$result = mysqli_query($conn, "SELECT * FROM books WHERE distribution='M' ORDER BY id ASC LIMIT $start_from, $limit");
...
<body>
    ...
    <section class="Best">
        <div class=" title">
            <h1 class="product_text"><strong><span class="den">Cook</span>Books</strong></h1>
        </div>
        <div class="clear"></div>
        <?php
        mysqli_set_charset($conn, "utf8");
        while ($row = mysqli_fetch_array($result)) { ?>
            <div class="boxx ">
                <div class="overlay">
                    <a href="../content.php?uid=<?php echo htmlentities($row["id"]) ?>"> <img src="<?php echo "../images/arabicbook/" . $row["picture"];  ?>" alt="" width="150px" height="250px"></a>
                    <div class="fff">
    ...
    <?php
    $result_db = mysqli_query($conn, "SELECT COUNT(id) FROM books WHERE distribution='M' ");
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        $pagLink .= "<li class='page-item'><a class='page-link' href='cookbooks.php?page=" . $i . "'>" . $i . "</a></li>";
    }
    echo $pagLink . "</ul>";
    ?>
