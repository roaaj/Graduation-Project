...
$res = mysqli_query($conn, "SELECT * FROM books WHERE distribution='P' ORDER BY id ASC LIMIT $start_from, $limit");
mysqli_set_charset($conn, "utf8");
// 
...
<body>
    <section class="Best">
        <div class=" title">
            <h1 class="product_text"><strong><span class="den">Human</span>development</strong></h1>
        </div>
        <div class="clear"></div>
        <?php
        while ($row = mysqli_fetch_array($res)) { ?>
            <div class="boxx ">
                <div class="overlay">
                    <a href="../content.php?uid=<?php echo $row["id"] ?>"> <img src="<?php echo "../images/arabicbook/" .  htmlentities($row["picture"]);  ?>" alt="" width="150px" height="250px"></a>
                    <div class="fff">
    ...
    <?php
    $result_db = mysqli_query($conn, "SELECT COUNT(id) FROM books  WHERE distribution='P'");
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        $pagLink .= "<li class='page-item'><a class='page-link' href='Humandevelopment.php?page=" . $i . "'>" . $i . "</a></li>";
    }
    echo $pagLink . "</ul>";
    ?>
