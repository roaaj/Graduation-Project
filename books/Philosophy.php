...
$res = mysqli_query($conn, "SELECT * FROM books WHERE distribution='F' ORDER BY id ASC LIMIT $start_from, $limit");
// 
...
<body>
    <section class="Best">
        <div class=" title">
            <h1 class="product_text"><strong><span class="den">Philosophy</span>Books</strong></h1>
        </div>
        <div class="clear"></div>
        <?php
        mysqli_set_charset($conn, "utf8");
    ...
    <?php
    $result_db = mysqli_query($conn, "SELECT COUNT(id) FROM books  WHERE distribution='F'");
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        $pagLink .= "<li class='page-item'><a class='page-link' href='Philosophy.php?page=" . $i . "'>" . $i . "</a></li>";
    }
    echo $pagLink . "</ul>";
    ?>
