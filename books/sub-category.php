...
$result = mysqli_query($conn, "SELECT * FROM books WHERE subcategory='" . $_SESSION['x'] . "'ORDER BY id ASC LIMIT $start_from, $limit");
...
<body>
    ...
    <?php
    $x = mysqli_query($conn, "select * from subcategory where id='" . $_SESSION['x'] . "'");
    mysqli_set_charset($conn, "utf8");
    while ($row = mysqli_fetch_array($x)) { ?>
        <section class="Best">
            <div class=" title">
                <h1 class="product_text"><strong><span class="den"><?php echo $row["subcategory"] . "/"; ?></span>Books</strong></h1>
            </div>
            <div class="clear"></div>
        <?php }
    mysqli_set_charset($conn, "utf8");
    while ($row = mysqli_fetch_array($result)) { ?>
        ...
        <?php
        $result_db = mysqli_query($conn, "SELECT COUNT(id) FROM books WHERE subcategory='" . $_SESSION['x'] . "' ");
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            $pagLink .= "<li class='page-item'><a class='page-link' href='http://localhost/final%20p/books/sub-category.php?id=" . $_SESSION['x'] . "&page=" . $i . "'>" . $i . "</a></li>";
        }
        echo $pagLink . "</ul>";
        ?>
